...
// Overlay definition
DEFINE_RSP_UCODE(rsp_blend_port);

void rsp_blend_init(void) {
  // Initialize if rspq (if it isn't already). It's best practice to let all
  // overlays always call rspq_init(), so that they can be themselves
  // initialized in any order by the user.
  rspq_init();
  ovl_id = rspq_overlay_register(&rsp_blend_port);
}
...
int main(void) {
  debug_init_isviewer();
  debug_init_usblog();
  display_init(RESOLUTION_640x480, DEPTH_16_BPP, 2, GAMMA_NONE,
               FILTERS_DISABLED);
  ...
  while (1) {
    ...
    rdpq_text_printf(NULL, MYFONT, 40, 20,
                     "Additive blending with %s (press A to toggle) -- %d us",
                     use_rdp ? "RDP" : "RSP", TIMER_MICROS(last_frame));
    ...
    if (use_rdp) {
      ...
      // Add a fence. This makes the RSP wait until the RDP has finished
      // drawing, which is what we need as we are going to process the pixels of
      // the background with the RSP.
      rdpq_fence();
    ...
    // Wait until RSP+RDP are idle. This is normally not required, but we force
    // it here to measure the exact frame computation time.
    rspq_wait();
